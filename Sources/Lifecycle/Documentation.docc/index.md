...
Use ``ServiceLifecycle/start(_:)`` to start the application.
...
``ServiceLifecycle/shutdown(_:)`` is an asynchronous operation. Errors will be logged and bubbled up to the provided completion handler.
